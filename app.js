...
(function(){if(window.hyperstackOnboardingLoaded)return;window.hyperstackOnboardingLoaded=true;class HyperStackOnboarding{constructor(){this.isFirstTime=!localStorage.getItem("hyperstack_has_cards")}start(){setTimeout(()=>{if(this.isFirstTime){this.show()}},5000)}show(){const existing=document.getElementById("hs-onboard-modal");if(existing)existing.remove();const modal=document.createElement("div");modal.id="hs-onboard-modal";modal.style.cssText="position:fixed;top:0;left:0;right:0;bottom:0;z-index:10000;display:flex;align-items:center;justify-content:center;font-family:system-ui,-apple-system,sans-serif;";modal.innerHTML=`<div style="position:absolute;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.9);backdrop-filter:blur(12px);"></div><div style="position:relative;background:#0a0a0a;border:1px solid #333;border-radius:16px;padding:48px;max-width:580px;width:90%;box-shadow:0 25px 80px rgba(0,0,0,0.6);"><div style="margin-bottom:32px;"><h2 style="margin:0 0 16px 0;font-size:32px;font-weight:700;color:#fff;line-height:1.2;">Set up your memory</h2><p style="margin:0;font-size:17px;line-height:1.6;color:#999;">Describe your project and I will create memory cards for your AI agents.</p></div><textarea id="hs-project-desc" placeholder="Example: Building a marketing agency for content creators. Using Next.js, Supabase, and Stripe." rows="4" style="width:100%;padding:16px;background:#000;border:1px solid #333;border-radius:12px;color:#fff;font-size:15px;margin:0 0 24px 0;font-family:inherit;resize:vertical;box-sizing:border-box;line-height:1.6;" onfocus="this.style.borderColor='#3b82f6'" onblur="this.style.borderColor='#333'"></textarea><div style="background:#0f1419;border-left:3px solid #3b82f6;padding:16px 20px;margin:0 0 32px 0;border-radius:8px;font-size:14px;line-height:1.7;color:#9ca3af;"><strong style="color:#fff;">Include:</strong> Tech stack, Frameworks, Databases</div><div style="display:flex;gap:16px;"><button id="hs-skip-btn" style="flex:1;padding:16px 28px;border:1px solid #333;background:transparent;color:#999;border-radius:12px;font-size:15px;font-weight:600;cursor:pointer;">Skip</button><button id="hs-create-btn" style="flex:2;padding:16px 28px;border:none;background:#3b82f6;color:#fff;border-radius:12px;font-size:15px;font-weight:600;cursor:pointer;">Create Cards</button></div></div>`;document.body.appendChild(modal);document.getElementById("hs-skip-btn").onclick=()=>this.skip();document.getElementById("hs-create-btn").onclick=()=>this.create()}async create(){const description=document.getElementById("hs-project-desc")?.value?.trim();if(!description){alert("Please describe your project");return}const modal=document.getElementById("hs-onboard-modal");const modalContent=modal.querySelector("div[style*='position:relative']");modalContent.innerHTML=`<div style="text-align:center;padding:60px 40px;"><div style="margin:0 auto 32px;border:4px solid #1a1a1a;border-top:4px solid #3b82f6;border-radius:50%;width:64px;height:64px;animation:hs-spin 1s linear infinite;"></div><h3 style="margin:0;font-size:24px;font-weight:600;color:#fff;">Creating cards...</h3></div>`;if(!document.getElementById("hs-spinner-style")){const style=document.createElement("style");style.id="hs-spinner-style";style.textContent="@keyframes hs-spin{to{transform:rotate(360deg)}}";document.head.appendChild(style)}try{const cards=this.parseDescription(description);const token=localStorage.getItem("token");const createdCards=[];for(const card of cards){try{const response=await fetch("https://hyperstack-cloud.vercel.app/api/cards?workspace=default",{method:"POST",headers:{Authorization:`Bearer ${token}`,"Content-Type":"application/json"},body:JSON.stringify(card)});if(response.ok){createdCards.push({slug:card.slug,title:card.title,cardType:card.cardType})}}catch(err){console.error(err)}}if(createdCards.length>0){this.showSuccess(createdCards)}else{this.showError("Could not create cards")}}catch(error){this.showError("Something went wrong")}}parseDescription(text){const timestamp=Date.now();const cards=[];const techKeywords=["next.js","react","vue","supabase","firebase","postgresql","mysql","mongodb","stripe","clerk"];const foundTech=[];techKeywords.forEach(tech=>{if(text.toLowerCase().includes(tech)){foundTech.push(tech)}});if(foundTech.length>0){cards.push({slug:`tech-${timestamp}`,title:"Tech Stack",body:`Using ${foundTech.join(", ")}`,cardType:"project",stack:"projects",keywords:foundTech,sourceAgent:"dashboard",targetAgent:"all"})}cards.push({slug:`project-${timestamp}`,title:"Project Overview",body:text.substring(0,250),cardType:"project",stack:"projects",keywords:["project"],sourceAgent:"dashboard",targetAgent:"all"});return cards}showSuccess(cards){const modal=document.getElementById("hs-onboard-modal");const modalContent=modal.querySelector("div[style*='position:relative']");modalContent.innerHTML=`<h2 style="margin:0 0 16px 0;font-size:32px;font-weight:700;color:#fff;">Created ${cards.length} cards</h2><div style="margin:24px 0;">${cards.map(c=>`<div style="background:#000;border-left:3px solid #3b82f6;padding:16px;margin-bottom:12px;border-radius:8px;"><div style="font-weight:600;color:#fff;">${c.title}</div><div style="font-size:13px;color:#6b7280;">${c.cardType}</div></div>`).join("")}</div><button id="hs-complete-btn" style="width:100%;padding:16px;border:none;background:#3b82f6;color:#fff;border-radius:12px;font-weight:600;cursor:pointer;">Go to Dashboard</button>`;document.getElementById("hs-complete-btn").onclick=()=>this.complete();localStorage.setItem("hyperstack_has_cards","true")}showError(msg){const modal=document.getElementById("hs-onboard-modal");const modalContent=modal.querySelector("div[style*='position:relative']");modalContent.innerHTML=`<h2 style="margin:0 0 16px 0;font-size:32px;color:#fff;">Something went wrong</h2><p style="color:#ef4444;">${msg}</p><button id="hs-skip-error-btn" style="width:100%;padding:16px;background:#3b82f6;color:#fff;border:none;border-radius:12px;cursor:pointer;">Continue</button>`;document.getElementById("hs-skip-error-btn").onclick=()=>this.skip()}skip(){localStorage.setItem("hyperstack_has_cards","true");this.complete()}complete(){const modal=document.getElementById("hs-onboard-modal");if(modal)modal.remove()}}window.hyperstackOnboard=new HyperStackOnboarding();setTimeout(()=>{if(!localStorage.getItem("hyperstack_has_cards")){window.hyperstackOnboard.start()}},5000)})();