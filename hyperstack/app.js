...
// CONVERSATIONAL ONBOARDING - Dark Theme Styled
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
class ConversationalOnboarding {
  ...
  show() {
    ...
    modal.innerHTML = `
      <div style="position:absolute;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.85);backdrop-filter:blur(8px);" onclick="event.target === this && conversationalOnboard.skip()"></div>
  ...
  async create() {
    ...
    if (!description) {
      this.showError('Please describe your project first');
      return;
    }
    ...
    modalContent.innerHTML = `
      ...
      <div style="text-align:center;padding:50px 0;">
        <div style="margin:0 auto;border:4px solid #1a1a1a;border-top:4px solid #3b82f6;border-radius:50%;width:56px;height:56px;animation:spin 1s linear infinite;"></div>
...
function tryShowOnboarding() {
  if (!localStorage.getItem('hyperstack_onboarded') && typeof conversationalOnboard !== 'undefined') {
    conversationalOnboard.start();
  }
}

setTimeout(tryShowOnboarding, 1000);
setTimeout(tryShowOnboarding, 2000);
setTimeout(tryShowOnboarding, 3000);

if (typeof go === 'function') {
  const originalGo = go;
  window.go = function(page) {
    originalGo(page);
    setTimeout(tryShowOnboarding, 800);
  };
}
